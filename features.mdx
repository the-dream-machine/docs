...
When there are multiple ways to complete a task, mansa will prompt the developer to select their preferred option instead of guessing and going down the wrong path.

## Easy for developers to use

The command `mansa install ...` is a drop-in replacement for `npm install ...`.
It fits into the current development workflow without requiring any behavior change.
...
For commands that are long-running, mansa will copy the command to the clipboard so the developer can paste it in a new terminal tab/window.
...
Sometimes, developers may want to tweak a few things just the way they like it. If that's the case, they can hit **tab** to bring up a chat box and request changes before they move on to the next step.
